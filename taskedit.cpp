...
TaskEdit::TaskEdit(QWidget *parent) : QWidget(parent), ui(new Ui::TaskEdit), m_edit(false) {
    ...
    // setup comboboxes
    this->connect<void (QComboBox::*)(int)>(this->ui->typeCombo, &QComboBox::activated, this, [this](int index) {
    ...
    // add button action
    this->connect(this->ui->addButton, &QPushButton::clicked, this, [this, emptyName]() {
        ...
        if (!this->isEditing()) {
            Task::instance()->add(taskName, this->ui->pointsInteger->value(), this->ui->multiInteger->value(),
                static_cast<Task::Types>(this->ui->typeCombo->currentIndex()), static_cast<Task::Styles>(this->ui->styleCombo->currentIndex()),
