...
TaskToolBar::TaskToolBar(QWidget *parent) : ToolBar(parent) {
    ...
    auto move = [this](bool up) {
        ...
        if (reindex) {
            ...
            // get id list
            for (int y = 0; y < Task::instance()->count(); y++) idList << Task::instance()->id(Task::instance()->row(y));

            // reorder tasks accordint to id list
            y = 0;
            for (const Id id : std::as_const(idList)) {
        ...
        QTableView *container(EditorDialog::instance()->container);
        ...
        if (EditorDialog::instance()->isDockVisible() || !index.isValid() || !other.isValid()) return;
    ...
    addAction(QIcon::fromTheme("export"), tr("Export tasks"), [this]() {
        ...
        if (csv.open(QFile::WriteOnly | QFile::Truncate)) {
            ...
            for (int y = 0; y < Task::instance()->count(); y++) {
                ...
                out << QString("%1;%2;%3;%4;%5;%6%7")
                           .arg(Task::instance()->name(row).replace(";", " |"), Task::instance()->description(row).replace(";", "  |"),
                               QString::number(static_cast<int>(Task::instance()->type(row))), QString::number(static_cast<int>(Task::instance()->style(row))),
                               Task::instance()->type(row) == Task::Types::Multi ? QString::number(Task::instance()->multi(row)) : "",
                               QString::number(Task::instance()->points(row)), "\n");
            }
        }
    ...
    addAction(QIcon::fromTheme("export"), tr("Export as xlsx"), [this]() {
        ...
        for (int y = 0; y < Task::instance()->count(); y++) {
            ...
            QString style;
            switch (Task::instance()->style(row)) {
            case Task::Styles::Regular: simple << out; break;

            case Task::Styles::Bold: difficult << out; break;

            case Task::Styles::Italic: other << out; break;

            case Task::Styles::NoStyle: break;
            }
        }
            ...
            while (!stream.atEnd()) {
                ...
                if (parms.length() != 6) {
                    QMessageBox::critical(
                        this, TaskToolBar::tr("Error"), TaskToolBar::tr("Could not parse CSV file, numParms=%1 (required 6)").arg(parms.length()));
                    return;
                }
