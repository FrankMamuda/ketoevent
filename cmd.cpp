...
Cmd::Cmd(QObject *parent) : QObject(parent) {
    ...
    add("test_setup", static_cast<void (*)(const QString &, const QStringList &)>([](const QString &, const QStringList &args) {
        ...
        for (y = 0; y < numTasks; y++)
            Task::instance()->add(
                "Task " + QString::number(y), rand() % 20 + 1, rand() % 5 + 1, static_cast<Task::Types>(rand() % 2), static_cast<Task::Styles>(rand() % 3), "");

        // add teams
        for (y = 0; y < numTeams; y++) {
            Row team = Team::instance()->add("Team " + QString::number(y), rand() % EventTable::DefaultMaxMembers + 1, QTime(), "Test");
            if (team == Row::Invalid) continue;
...
bool Cmd::tokenize(const QString &string, QString &command, QStringList &args) {
    ...
    for (const QString &str : list) {
        ...
        if (command.isEmpty()) command = token;
        else args << token;
    }
