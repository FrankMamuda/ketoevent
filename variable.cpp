...
QString Variable::bind( const QString &key, QObject *object ) {
    ...
    if ( !QString::compare( object->metaObject()->className(), "QCheckBox" )) {
        ...
        qCWarning( VariableDebug::category, this->tr( "unsupported container \"%1\"" ).arg( object->metaObject()->className()).toUtf8().constData());
    }
    ...
    if ( !QString::compare( object->metaObject()->className(), "QCheckBox" )) {
        ...
        if ( comboBox != nullptr ) {
            if ( comboBox->currentIndex() != -1 ) {
                ...
                qCWarning( VariableDebug::category, this->tr( "void comboBox for variable \"%1\"" ).arg( key ).toUtf8().constData());
            }
        }
    } else {
        qCWarning( VariableDebug::category, this->tr( "unsupported container \"%1\"" ).arg( object->metaObject()->className()).toUtf8().constData());
